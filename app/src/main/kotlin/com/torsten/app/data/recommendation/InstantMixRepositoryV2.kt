...
class InstantMixRepositoryV2(
    ...
    suspend fun getMix(seedSong: SongEntity, artistName: String): List<SongEntity> {
        ...
        val lbEntries = if (mbid != null) {
            listenBrainzClient.getArtistRadio(mbid).also {
                Timber.tag(tag).d("LB radio returned ${it.size} entries for '$artistName'")
            }
        } else {
            emptyList()
        }
        ...
        Timber.tag(tag).d("LB step: $lbMatchCount songs from ${similarArtistNames.size} similar artists")

        // ── 4. Subsonic fallback if LB empty or thin local match ──────────────
        val needsFallback = lbEntries.isEmpty() || lbMatchCount < 10
        Timber.tag(tag).d(
        ...
        if (needsFallback && currentMixToken.get() == myToken) {
            ...
            if (config.isConfigured) {
                ...
                val firstBatch = runCatching { client.getSimilarSongs2(seedSong.id, 50) }
                    .getOrElse { emptyList() }
                Timber.tag(tag).d("Subsonic batch-1: ${firstBatch.size} candidates")
                upsertAndMerge(firstBatch, usedSongIds, entityMap, pool)
                Timber.tag(tag).d("Pool after batch-1: ${pool.size} (LB=$lbMatchCount Subsonic=${pool.size - lbMatchCount})")

                // ── 4b. Second batch when pool is still thin ──────────────────
                // Seed on the highest-ranked song already in the pool (first LB match if
                // any, otherwise first Subsonic result) to get a complementary set.
