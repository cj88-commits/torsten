 ...
 * Pre-populate [albumSongs] or [songs] before the test; all other methods are no-ops.
 */
class FakeSongDao(
    private val albumSongs: Map<String, List<SongEntity>> = emptyMap(),
    private val songs: List<SongEntity> = emptyList(),
) : SongDao {
    ...
    override suspend fun getSongsByArtistName(artistName: String): List<SongEntity> =
        songs.filter { it.artistName == artistName || it.albumArtistName == artistName }
            .sortedWith(compareBy({ it.albumId }, { it.trackNumber }))
